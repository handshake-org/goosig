...
describe('Group Ops', function() {
  ...
  it('should compute inv2 (t1)', () => {
    ...
    assert.bigIntEqual(t1.reduce(BigMath.mod(e1 * e1Inv, t1.n)), 1n);
    assert.bigIntEqual(t1.reduce(BigMath.mod(e2 * e2Inv, t1.n)), 1n);
  });
  ...
  it('should compute inv2 (t2)', () => {
    ...
    assert.bigIntEqual(t2.reduce(BigMath.mod(e1_s * e1_sInv, t2.n)), 1n);
    assert.bigIntEqual(t2.reduce(BigMath.mod(e2_s * e2_sInv, t2.n)), 1n);
  });
  ...
  it('should compute inv5 (t1)', () => {
    ...
    for (const [e, eInv] of testUtil.zip(eVals, eInvs))
      assert.bigIntEqual(t1.reduce(BigMath.mod(e * eInv, t1.n)), 1n);
  });
  ...
  it('should compute inv5 (t2)', () => {
    ...
    for (const [e, eInv] of testUtil.zip(eVals, eInvs))
      assert.bigIntEqual(t2.reduce(BigMath.mod(e * eInv, t2.n)), 1n);
  });
});
