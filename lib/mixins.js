...
class CombPrecomp {
  ...
  to_comb_exp(e) {
    ...
    assert(ebits.length === this.nbits);

    for (let j = 0; j < nsh_tot; j++)
      ebits_split.push(ebits.slice(j * nsh, (j + 1) * nsh));

    for (let i = this.adds_per_shift - 1; i >= 0; i--)
      iters.push(this.iter_bits(ebits_split, i));

    return util.zip(...iters);
  }

  *iter_bits(ebits_split, i) {
    assert(Array.isArray(ebits_split));
    assert((i >>> 0) === i);

    const chunks = [];

    for (let j = 0; j < this.points_per_add; j++) {
      const chunk = ebits_split[i + j * this.adds_per_shift];
      chunks.push(chunk);
    }

    for (const x of util.zip(...chunks))
      yield this._from_win(x);
  }
  ...
  powgh(e1, e2) {
    ...
    for (const [e1vs, e2vs] of util.zip(e1e, e2e)) {
      ...
      for (const [idx, [e1v, e2v]] of util.enumerate(util.zip(e1vs, e2vs))) {
        if (e1v !== 0) {
          const g = gcomb.items[idx * gcomb.points_per_subcomb + e1v - 1];
          ret = this.mul(ret, g);
        }

        if (e2v !== 0) {
          const h = hcomb.items[idx * hcomb.points_per_subcomb + e2v - 1];
          ret = this.mul(ret, h);
        }
      }
    }
...
class WnafMixin extends CombMixin {
  ...
  _wnaf(r, ws, bitlen = null) {
    ...
    for (let i = bitlen - 1; i >= 0; i--) {
      ...
      assert(Number.isSafeInteger(out[i]));
  ...
  pow2(b1, b1Inv, e1, b2, b2Inv, e2) {
    ...
    for (const [w1, w2] of util.zip(e1bits, e2bits)) {
      if (!this.equals(ret, this.id))
        ret = this.sqr(ret);
