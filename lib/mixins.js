...
class CombPrecomp {
  ...
  init(g, combspec, gops) {
    ...
    const [ppa, aps, nshifts, bpw, _, size] = combspec;
...
class CombMixin {
  ...
  powgh(e1, e2) {
    ...
    const loge = Math.max(bm.bitLength(e1), bm.bitLength(e2));

    let gcomb = null;
    let hcomb = null;

    for (const c in this.combs) {
      if (loge <= c[0].nbits) {
        [gcomb, hcomb] = c;
        break;
      }
...
class WnafMixin extends CombMixin {
  ...
  pow2(b1, b1Inv, e1, b2, b2Inv, e2) {
    ...
    const totlen = Math.max(bm.bitLength(e1), bm.bitLength(e2)) + 1;
