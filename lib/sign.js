...
const {umod, modPow, decode} = BigMath;
...
class GooSigner {
  ...
  sign(C1, s, msg) {
    ...
    assert(equals(C1, this.gops.reduce(this.gops.powgh(this.n, s))),
    ...
    for (t of defs.primes) {
      w = util.sqrt_modn(t, this.p, this.q);
      if (w != null)
        break;
    }
...
function equals(x, y) {
  assert(typeof x === 'bigint' || Array.isArray(x));
  assert(typeof y === 'bigint' || Array.isArray(y));
  ...
  assert(typeof x[0] === 'bigint');
  assert(typeof x[1] === 'bigint');
  assert(typeof x[2] === 'bigint');
  assert(typeof y[0] === 'bigint');
  assert(typeof y[1] === 'bigint');
  assert(typeof y[2] === 'bigint');
