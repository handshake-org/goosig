...
class RSAGroupOps extends RandMixin { // XXX maybe change inheritance chain
  ...
  inv2(b1, b2) {
    ...
    return [umod(b2 * b12Inv, this.n), umod(b1 * b12Inv, this.n)];
  }
...
class ClassGroupOps extends RandMixin { // XXX maybe change inheritence chain
  ...
  mul(m1, m2) {
    assert(Array.isArray(m1));
    assert(m1.length === 3);
    assert(typeof m1[0] === 'bigint');
    assert(typeof m1[1] === 'bigint');
    assert(typeof m1[2] === 'bigint');
    assert(Array.isArray(m2));
    assert(m2.length === 3);
    assert(typeof m2[0] === 'bigint');
    assert(typeof m2[1] === 'bigint');
    assert(typeof m2[2] === 'bigint');
    ...
    assert(this.discrim(ret) === this.D);
  ...
  sqr(m) {
    ...
    assert(m.length === 3);
    assert(typeof m[0] === 'bigint');
    assert(typeof m[1] === 'bigint');
    assert(typeof m[2] === 'bigint');
