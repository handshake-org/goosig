...
class Goo {
  constructor(n, g, h, modBits) {
    ...
    this.e1bits = new Int32Array(constants.ELL_BITS + 1);
    this.e2bits = new Int32Array(constants.ELL_BITS + 1);
    this.combs = [];
  ...
  powWnaf(b, bInv, e) {
    ...
    if (totlen > constants.ELL_BITS + 1)
  ...
  inv2(b1, b2) {
    ...
    const b12Inv = b1.redMul(b2).redInvert();
  ...
  inv7(b1, b2, b3, b4, b5, b6, b7) {
    ...
    const b1234567Inv = b1234567.redInvert();
  ...
  recon(b1, b1Inv, e1, b2, b2Inv, e2, e3, e4) {
    const lhs = this.pow2(b1, b1Inv, e1, b2, b2Inv, e2);
    const rhs = this.powgh(e3, e4);
    const res = this.mul(lhs, rhs);
  ...
  fsChal(C1, C2, C3, t, A, B, C, D, E, msg, verify) {
    ...
    let ell = prng.randomBitsNZ(constants.ELL_BITS);
  ...
  _sign(msg, s_prime, p, q) {
    ...
    const r_w = this.randomScalar();
    const r_w2 = this.randomScalar();
    const r_a = this.randomScalar();
    const r_an = this.randomScalar();
    const r_s1w = this.randomScalar();
    const r_sa = this.randomScalar();
    const r_s2 = this.randomScalar();
    ...
    while (ell == null || ell.bitLength() !== constants.ELL_BITS) {
    ...
    const Dq = this.reduce(this.mul(this.pow(C1Inv, C1, z_a.div(ell)),
                                    this.powgh(z_an.div(ell), z_sa.div(ell))));
    const Eq = z_w2.sub(z_an).div(ell);

    assert(Eq.sign() >= 0);
    assert(Eq.bitLength() <= constants.EXPONENT_SIZE);
    ...
    if (chal.sign() === 0
        ...
        || z_s2.sign() === 0) {
      // Try again.
      return this._sign(msg, s_prime, p, q);
    }
  ...
  _verify(msg, sig, C1) {
    ...
    let {C2, C3, t, chal, ell, Aq, Bq, Cq, Dq, Eq,
         z_w, z_w2, z_s1, z_a, z_an, z_s1w, z_sa, z_s2} = sig;

    // Make sure that the public key is valid.
    // `t` must be one of the small primes in our list.
    if (t.sign() <= 0 || !primes.smallPrimes.includes(t.toNumber()))
      throw new Error('Invalid small prime.');

    // `chal` must be in range.
    if (chal.sign() <= 0 || chal.bitLength() > constants.CHAL_BITS)
      throw new Error('Invalid chal value.');

    // `ell` must be in range.
    if (ell.sign() <= 0 || ell.bitLength() > constants.ELL_BITS)
      throw new Error('Invalid ell value.');

    // `ell` must be odd.
    if (!ell.isOdd())
      throw new Error('Non-prime ell value.');

    // All group elements must be the "canonical"
    // element of the quotient group (Z/n)/{1,-1}.
    if (C1.sign() <= 0 || !this.isReduced(C1)
        || C2.sign() <= 0 || !this.isReduced(C2)
        || C3.sign() <= 0 || !this.isReduced(C3)
        || Aq.sign() <= 0 || !this.isReduced(Aq)
        || Bq.sign() <= 0 || !this.isReduced(Bq)
        || Cq.sign() <= 0 || !this.isReduced(Cq)
        || Dq.sign() <= 0 || !this.isReduced(Dq)) {
      throw new Error('Non-reduced parameters.');
    }

    // `Eq` must be in range.
    if (Eq.sign() <= 0 || Eq.bitLength() > constants.EXPONENT_SIZE)
      throw new Error('Invalid Eq value.');

    // `z_prime` must be within range.
    if (z_w.sign() <= 0 || z_w.cmp(ell) >= 0
        || z_w2.sign() <= 0 || z_w2.cmp(ell) >= 0
        || z_s1.sign() <= 0 || z_s1.cmp(ell) >= 0
        || z_a.sign() <= 0 || z_a.cmp(ell) >= 0
        || z_an.sign() <= 0 || z_an.cmp(ell) >= 0
        || z_s1w.sign() <= 0 || z_s1w.cmp(ell) >= 0
        || z_sa.sign() <= 0 || z_sa.cmp(ell) >= 0
        || z_s2.sign() <= 0 || z_s2.cmp(ell) >= 0) {
    ...
    const A = this.recon(Aq, AqInv, ell, C2Inv, C2, chal, z_w, z_s1);
    const B = this.recon(Bq, BqInv, ell, C3Inv, C3, chal, z_a, z_s2);
    const C = this.recon(Cq, CqInv, ell, C2Inv, C2, z_w, z_w2, z_s1w);
    const D = this.recon(Dq, DqInv, ell, C1Inv, C1, z_a, z_an, z_sa);
