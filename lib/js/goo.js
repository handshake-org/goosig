...
class Goo {
  ...
  init(maxCombSize, modBits) {
    ...
    if (modBits != null) {
      if (modBits < constants.MIN_RSA_BITS || modBits > constants.MAX_RSA_BITS)
  ...
  challenge(key) {
    ...
    const [s_prime, C1] = this._challenge(n);
    const bytes = (this.bits + 7) >>> 3;

    return [
      BigMath.encode(s_prime, 32),
      BigMath.encode(C1, bytes)
    ];
  }
  ...
  sign(msg, s_prime, C1, key) {
    ...
    if (C1.length !== BigMath.byteLength(this.n))
  ...
  _sign(msg, s_prime, C1, n, p, q) {
    ...
    if (s_prime <= 0n || C1 <= 0n || n <= 0n || p <= 0n || q <= 0n)
    ...
    if (Dq < 0n || BigMath.bitLength(Dq) > 2048)
      throw new Error(`Invalid Dq: (${z_w2} - ${z_an}) / ${ell} = ${Dq}.`);
  ...
  _verify(msg, sig, C1) {
    ...
    if (D < 0n)
      throw new Error(`Negative D value: ${D}.`);
...
class Comb {
  ...
  toJSON() {
    return {
      base: BigMath.encodeHex(this.items[0]),
...
function hashAll(items) {
  ...
  for (const item of items) {
    ...
    // Sanity check.
    assert(len <= ((constants.MAX_RSA_BITS + 7) >>> 3));
