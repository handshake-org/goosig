...
class Goo {
  ...
  oneMul(ret, w, p, n) {
    if (w > 0)
      ret = ret.redIMul(p[(w - 1) >>> 1]);
    else if (w < 0)
      ret = ret.redIMul(n[(-1 - w) >>> 1]);
  ...
  derive(C1, C2, C3, t, A, B, C, D, E, msg) {
    ...
    ctx.update(Buffer.from([E.isNeg() ? 1 : 0]));
    ctx.update(msg);
  ...
  expandSprime(s_prime) {
    const prng = new PRNG(s_prime);
    return prng.randomBits(constants.EXPONENT_SIZE);
  }
  ...
  _sign(msg, s_prime, p, q) {
    ...
    for (let i = 0; i < smalls.length; i++) {
      ...
      const j = rng.randomRange(0, smalls.length - i);
  ...
  _verify(msg, S, C1) {
    ...
    // `Eq` must be in range.
    if (Eq.bitLength() > constants.EXPONENT_SIZE)
    ...
    const z_w2_m_an = z_w2.sub(z_an).mod(ell);
...
class Comb {
  ...
  init(base, spec) {
    ...
    const skip = (1 << ppa) - 1;
    const win = BN.shift(1, bpw);
    const pow = BN.shift(1, shifts);
    ...
    for (let i = 1; i < aps; i++) {
      for (let j = 0; j < skip; j++) {
        ...
        this.items[k] = this.items[k - skip].redPow(pow);
      }
    }
  ...
  static generate(bits, maxSize) {
    ...
    const generate = (shifts, aps, ppa, bpw) => {
      ...
      const spec = [ppa, aps, shifts, bpw, ops, size];
...
function isSanePrivateKey(key) {
  ...
  const plen = countLeft(key.p);
  const qlen = countLeft(key.q);
