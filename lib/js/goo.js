...
class Goo {
  constructor(n, g, h, modBits) {
    ...
    this.gRaw = this.g.encode('be', 4);
    this.hRaw = this.h.encode('be', 4);
  ...
  oneMul(ret, w, pctabP, pctabN) {
    ...
    if (w > 0)
      ret = ret.redMul(pctabP[(w - 1) >>> 1]);
    else if (w < 0)
      ret = ret.redMul(pctabN[(-1 - w) >>> 1]);
  ...
  powWnaf(b, bInv, e) {
    ...
    let ret = BN.from(1).toRed(this.red);
  ...
  fsChal(C1, C2, C3, t, A, B, C, D, E, msg, verify) {
    ...
    ctx.update(C1.encode('be', modBytes));
    ctx.update(C2.encode('be', modBytes));
    ctx.update(C3.encode('be', modBytes));
    ctx.update(t.encode('be', 4));
    ctx.update(A.encode('be', modBytes));
    ctx.update(B.encode('be', modBytes));
    ctx.update(C.encode('be', modBytes));
    ctx.update(D.encode('be', modBytes));
  ...
  _sign(msg, s_prime, p, q) {
    ...
    const z_w2 = chal.mul(w).mul(w).iadd(r_w2);
    const z_s1 = chal.mul(s1).iadd(r_s1);
    const z_a = chal.mul(a).iadd(r_a);
    const z_an = chal.mul(a).mul(n).iadd(r_an);
    const z_s1w = chal.mul(s1).mul(w).iadd(r_s1w);
    const z_sa = chal.mul(s).mul(a).iadd(r_sa);
  ...
  _verify(msg, sig, C1) {
    ...
    const E = Eq.mul(ell).add(z_w2_m_an.sub(t.mul(chal)));
