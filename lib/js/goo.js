...
class Goo {
  ...
  randomScalar(rng) {
    ...
    const bits = Math.min(constants.EXPONENT_BITS, this.randBits);
  ...
  derive(C1, C2, C3, t, A, B, C, D, E, msg) {
    ...
    const expBytes = (constants.EXPONENT_BITS + 7) >>> 3;
    ...
    const key = ctx.final();
    const prng = PRNG.fromKey(key, constants.PRNG_DERIVE);
  ...
  expandSprime(s_prime) {
    const prng = PRNG.fromKey(s_prime, constants.PRNG_EXPAND);
    return prng.randomBits(constants.EXPONENT_BITS);
  }
  ...
  _sign(msg, s_prime, p, q) {
    ...
    assert(Eq.bitLength() <= constants.EXPONENT_BITS);
  ...
  _verify(msg, S, C1) {
    ...
    // `Eq` must be in range.
    if (Eq.bitLength() > constants.EXPONENT_BITS)
