...
class Sigma extends bio.Struct {
  constructor(chall, ell, Aq, Bq, Cq, Dq, z_prime) {
    super();

    assert(chall == null || typeof chall === 'bigint');
  ...
  getJSON() {
    return {
      chall: BigMath.encodeHex(this.chall),
  ...
  fromJSON(json) {
    assert(json && typeof json === 'object');
    this.chall = BigMath.decodeHex(json.chall);
