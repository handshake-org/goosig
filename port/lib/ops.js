...
class RSAGroupOps extends RandMixin {
  ...
  powgh_slow(e1, e2) {
    const q1 = modPow(this.g, e1, this.n);
    const q2 = modPow(this.h, e2, this.n);
    return umod(q1 * q2, this.n);
  }

  pow2_slow(b1, b1Inv, e1, b2, b2Inv, e2) {
    const q1 = modPow(b1, e1, this.n);
    const q2 = modPow(b2, e2, this.n);
    return umod(q1 * q2, this.n);
  }
  ...
  to_json() {
    return {
      ...
      g: BigMath.encodeHex(this.g),
      h: BigMath.encodeHex(this.h),
      combs: this.combs.map((combs) => {
        return combs.map(comb => comb.to_json());
      })
    };
  }
}
  ...
  reduce(f) {
    ...
    for (;;) {
      if (-a < b && b <= a) {
        if (a > c) {
          b = -b;
          // Using a one-line swap
          // apparently crashes v8.
          // [a, c] = [c, a];
          const t = a;
          a = c;
          c = t;
