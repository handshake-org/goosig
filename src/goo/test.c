...
rng_init(goo_prng_t *rng) {
  ...
  goo_prng_init(rng);
  goo_prng_seed(rng, &entropy[0], &entropy[32]);
}
...
run_hash_test(void) {
  ...
  assert(memcmp(&out[0], expect, 32) == 0);

  goo_sha256_init(&ctx);
  goo_sha256_update(&ctx, GOO_AOL1, sizeof(GOO_AOL1));
  goo_sha256_final(&ctx, &out[0]);

  assert(memcmp(&out[0], GOO_AOL1_HASH, 32) == 0);

  goo_sha256_init(&ctx);
  goo_sha256_update(&ctx, GOO_AOL2, sizeof(GOO_AOL2));
  goo_sha256_final(&ctx, &out[0]);

  assert(memcmp(&out[0], GOO_AOL2_HASH, 32) == 0);

  goo_sha256_init(&ctx);
  goo_sha256_update(&ctx, GOO_RSA2048, sizeof(GOO_RSA2048));
  goo_sha256_final(&ctx, &out[0]);

  assert(memcmp(&out[0], GOO_RSA2048_HASH, 32) == 0);
...
run_chacha20_test(void) {
  ...
  const unsigned char expect[32] = {
    0x01, 0x86, 0x96, 0xef, 0x4b, 0x93, 0x9d, 0xa6,
    0x00, 0x60, 0x69, 0xfb, 0x61, 0x8c, 0x09, 0x7f,
    0xfe, 0x80, 0x03, 0x27, 0x6c, 0xf4, 0x09, 0xea,
    0xc3, 0xde, 0x59, 0x7a, 0x44, 0xcb, 0x06, 0xcd
  };

  printf("Testing ChaCha20 (+ XChaCha20)...\n");

  memset(&out[0], 0x00, 32);
  memset(&key[0], 0x01, 32);
  memset(&nonce[0], 0x02, 24);

  goo_chacha20_init(&ctx, &key[0], &nonce[0]);

  for (i = 0; i < 1000; i++)
    goo_chacha20_encrypt(&ctx, &out[0], &out[0], 32);
...
run_prng_test(void) {
  ...
  assert(mpz_cmp_ui(x, 611248669) == 0);
  goo_prng_random_bits(&prng, x, 31);
  assert(mpz_cmp_ui(x, 1662132545) == 0);
  goo_prng_random_bits(&prng, x, 31);
  goo_prng_random_int(&prng, y, x);
  assert(mpz_cmp_ui(y, 1087011081) == 0);

  assert(goo_prng_random_num(&prng, 65537) == 48413);
  ...
  goo_prng_random_bits(&prng, x, 31);
  assert(mpz_cmp_ui(x, 1610517429) == 0);
